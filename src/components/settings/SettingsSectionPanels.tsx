...
export function SettingsSectionPanels({
  ...
  if (section === "connections") {
    const rawKey = sessionStorage.getItem("GLM_BYOK_KEY") || "";
    const hasKey = Boolean(rawKey);
    const maskedKey = hasKey && rawKey.length > 4 
        ...
        <SettingsStaggerItem>
          <AnimatedCard>
            <SettingsShellCard
              ...
              description="Configure custom API keys for cloud providers"
              icon={Bot}
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-xl bg-muted/20 border border-border/40 mt-4 gap-4">
                <div>
                  <p className="text-sm font-medium text-white">GLM API Key</p>
                  <p className="text-xs text-white/50">{hasKey ? `Configured (${maskedKey})` : "No key configured"}</p>
                </div>
                {hasKey ? (
                  <div className="flex gap-2 w-full sm:w-auto">
                    <Button 
                      ...
                      onClick={async () => {
                        try {
                          ...
                          else alert("Connection failed. Key might be invalid.");
                        } catch {
                    ...
                    <Button 
                      ...
                      size="sm"
                      onClick={() => {
                        sessionStorage.removeItem("GLM_BYOK_KEY");
                        localStorage.setItem("shadowtalk_byok_enabled", "false");
